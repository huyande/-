...
class BiaobController extends Controller {
	...
	public function index(){
		...
		$offset = $p*$pagesize; 
		$where['display'] = 0;
		$list = $m->join('LEFT JOIN qw_xcxbb_user ON qw_xcxbb_list.openid = qw_xcxbb_user.openid')->where($where)->order('qw_xcxbb_list.id DESC')->limit($offset.','.$pagesize)->select();
	...
	public function view(){
		$m = M('xcxbb_list');
		$map['id'] = I('id');
		$list = $m->join('LEFT JOIN qw_xcxbb_user ON qw_xcxbb_list.openid = qw_xcxbb_user.openid')->where($map)->select();

		$where['vid'] = I('id');
		$gzlists = M('xcxbb_guanzhu')->where($where)->order('time DESC')->limit(54)->select();
		if($list){
		   foreach($list as $key=>$value){
				 $list[$key]['datetime'] = date("m-d H:i", $value['datetime']);
	...
	public function getGuanzhu(){
		...
		if($cid == 1){
			if($openid != null){
				...
				$num = M('xcxbb_guanzhu')->where($where)->count();
				if($num > 0){
					$data['time'] = time();
					$gList = M('xcxbb_guanzhu')->where($where)->save($data);
					if($gList){
						echo "ok-save";
					}else{
						echo "no-save";
					...
					$data['time'] = time();
					$gList = M('xcxbb_guanzhu')->data($data)->add();
					if($gList){
						echo "ok-add";
					}else{
						echo "no-add";
			...
			$gzList = M('xcxbb_guanzhu')->where($where)->order('time DESC')->limit(55)->select();
			if($gzList){
	...
	public function up_message(){
		...
		$pagesize = I('page_size'); 
		$offset = $p*$pagesize; 
		$mList = M('xcxbb_message')->where($where)->order('id DESC')->limit($offset.','.$pagesize)->select();
		$num = M('xcxbb_message')->where($where)->count();
		if($num != 0){
			if($mList){
	...
	public function ad_message(){
		...
		if($content != null){
			$sList = M('xcxbb_message')->data($data)->add();
			if($sList){
				$where['id'] = I('vid');
				M('xcxbb_list')->where($where)->setInc('plnum');
	...
	public function delTxt(){
		$where['id'] = I('id');
		$data['display'] = 1;
		$op = M('xcxbb_list')->where($where)->save($data);
		if($op){
			echo 'ok';
		}else{
			echo 'no';
	...
	public function userMore(){
		$map['openid'] = I('openid');
		$op = M('xcxbb_user')->where($map)->find();
		if(!$op){
			echo "0";
			exit;
		}
	...
	public function userChick(){
		...
		$data['disuid'] = 0;
		$where['openid'] = I('openid');
		$op = M('xcxbb_user')->where($where)->find();
		if($op){
			echo "1";
		}else{
			M('xcxbb_user')->data($data)->add();
			echo "2";
		}
	}
	...
	public function addData(){
		...
		$data['datetime'] = time();
		$aid = M('xcxbb_list')->data($data)->add();
		if($aid){
			echo $aid;
		}else{
			echo $aid;
	...
	public function upload(){
		...
		if(IS_POST){
			...
			if(!$info) {
				...
				if(M('xcxbb_list')->where($where)->save($data)) {
					echo '0';
				}else{
					echo '1';
				}
